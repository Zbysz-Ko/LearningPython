...
def get_planszeo() -> list:
    """ Function retrieving all pages from https://planszeo.pl/okazje 
        and parse html data do list"""
    page = 1
    last = 1
    ...
    while page <= last:
        ...
        tis = time.time()
        response = get_data_from_web(url+str(page))
        tie = time.time()
        ...
        if page == 1:
            try:
                ...
                print("... found only one page (check it!)")

        games = soup.find_all("div", class_="grid grid-cols-12 gap-1 lg:gap-2")
        for game in games:
            result.append(
                [game.find("div", class_="text-xl w-full text-gray-600 font-extrabold").text,
                int(game.find_all("div", class_="text-lg")[0].text.strip()[1:].strip("%")),
                float(game.find("div", class_="text-purple-600 font-extrabold")\
                      .text.strip().strip(" zł").replace(",","."))])
        print("... delay 1 sec")
        time.sleep(1)
        page += 1
...
def choose_data(start: int, end: int) -> int:
    """ Function listing cache files and asking the user to select specific ones"""
    for j in range(start,end):
        print(f"    {j+1}. {datetime.strftime( datetime.strptime(cache_files[j][ln:ln+10], '%Y%m%d%H'),'%Y-%m-%d g.%H')}")
    while True:
...
if os.path.exists(path_cache):
    print("... Founded cache file for current hour, retrieving new data omitted")
else:
...
cache_files = glob.glob(os.path.basename(__file__).split('.')[0]
                        +"_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].cache")
...
if count >= 2:
    ...
    c1 = choose_data(0,count-1)

    print("\n... Please, choose older data: ")
    c2 = choose_data(c1,count)
